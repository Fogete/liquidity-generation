...
contract LiquidityGenerator {
    ...
    event Finalized(uint amountToken, uint amountETH);
    event Deposit(
    ...
    function finalize() public {
        ...
        uint _amountToken = token.myBalance();
        uint _amountETH = address(this).balance;

        uint _amountToken1 = _amountToken.mul(sharesRouter1).div(totalRouterShares);
        uint _amountETH1 = _amountETH.mul(sharesRouter1).div(totalRouterShares);
        uint _amountToken0 = _amountToken.sub(_amountToken1);
        uint _amountETH0 = _amountETH.sub(_amountETH1);

        token.safeApprove(router0, _amountToken0);
        token.safeApprove(router1, _amountToken1);
